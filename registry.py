...
def load_registry(path_or_buf) -> Registry:
    ...
    # ---- Match Review: carry forward any decision already made -----------
    ws = wb[SHEET_REVIEW]
    h = _headers(ws)
    if "Reviewer Decision" in h and "Product UUID" in h:
        dec_c, uuid_c = h["Reviewer Decision"], h["Product UUID"]
        corr_c = h.get("Corrected Masterlist ID")
        for r in ws.iter_rows(min_row=2, values_only=True):
            ...
            if dec == DECISION_LINK and corr_c is not None and corr_c < len(r):
                ids = split_ids(r[corr_c])
                if ids:
                    reg.locked[uuid] = LockedMatch(
                        mp_number=str(r[h.get("MP Number", -1)] or "").strip()
                        if "MP Number" in h else "",
                        product_uuid=uuid,
                        shop_sku=str(r[h.get("SKU Code", -1)] or "").strip()
                        if "SKU Code" in h else "",
                        frontend_name=str(r[h.get("Frontend Product Name (EN)", -1)] or "").strip()
                        if "Frontend Product Name (EN)" in h else "",
                        masterlist_ids=ids,
                    )
...
def build_plan(pos: PosReport, exp: IShopExport, reg: Registry,
    ...
    for l in exp.listings:
        ...
        plan.review_rows.append({
            ...
            "Reviewer Decision": reg.review_decisions.get(l.product_uuid, ""),
            "Notes": "",
            "_row_no": l.row_no,
            "_auto_exact": sugg[0].pos_row.stock_type_id if sugg and sugg[0].exact else "",
        })
    ...
    accounted = reg.linked_pos_ids() | reg.not_selling | reg.not_yet
    for p in usable_pos:
        ...
        plan.new_ml_rows.append({
            ...
            "Notes": reg.skipped.get(p.stock_type_id, ""),
        })
